...
do
    ...
    scp -o stricthostkeychecking=no -i waldoge_key_pair.pem -r copy_to_aws/lab3_code ubuntu@${public_ip}:~/
    scp -o stricthostkeychecking=no -i waldoge_key_pair.pem copy_to_aws/setup_lab3.sh ubuntu@${public_ip}:~/

    # setup environment and run the page on aws
    ssh -i waldoge_key_pair.pem ubuntu@${public_ip} screen -d -m './setup_lab3.sh'
done < "public_ip.txt"
